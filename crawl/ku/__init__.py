...
KU_DATA_PATH = os.path.join(DATA_PATH, 'ku')
PDF_SAVE_DIR = os.path.join(KU_DATA_PATH, "pdfs")
METADATA_JSON_PATH = os.path.join(KU_DATA_PATH, "metadata.json")

# Ensure log and save directory exists
os.makedirs(KU_DATA_PATH, exist_ok=True)
os.makedirs(PDF_SAVE_DIR, exist_ok=True)

# Setup logger for the package
LOGGER = setup_logging(
    log_file=os.path.join(KU_DATA_PATH, "ku_scrape.log"),
    level=logging.DEBUG
)

# Optional: define __all__ if other modules will use `from . import *`
__all__ = ["ROOT_PATH", "CRAWL_PATH", "DATA_PATH", "KU_DATA_PATH", "LOGGER"]
